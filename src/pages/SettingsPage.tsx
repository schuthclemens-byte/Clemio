...
import {
  ...
  Globe, Type, Contrast, SpellCheck, AlignLeft, Shield, ChevronRight, Settings2,
} from "lucide-react";
...
const SectionLabel = ({ icon: Icon, label }: { icon: React.ElementType; label: string }) => (
  <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-2 px-1 mb-2">
    ...
    <div className="flex flex-col min-h-screen bg-background" {...swipeHandlers}>
      ...
      <div className="flex-1 p-4 space-y-6 pb-28">
        ...
        <section className="animate-reveal-up">
          <SectionLabel icon={MessageSquareText} label={tr("Kommunikation", "Communication")} />
          ...
          {/* Push – compact */}
          {pushCap.canUsePush && (
            <div className="bg-card rounded-2xl shadow-sm overflow-hidden mt-2">
        ...
        <section className="animate-reveal-up" style={{ animationDelay: "30ms" }}>
          <SectionLabel icon={Volume2} label={tr("Wiedergabe", "Playback")} />
          <div className="bg-card rounded-2xl shadow-sm overflow-hidden">
          ...
          {/* Speech rate */}
          <div className="bg-card rounded-2xl shadow-sm overflow-hidden px-4 py-3.5 mt-2">
        ...
        <section className="animate-reveal-up" style={{ animationDelay: "60ms" }}>
          <SectionLabel icon={Globe} label={tr("Anzeige", "Display")} />
        ...
        <section className="animate-reveal-up" style={{ animationDelay: "90ms" }}>
          <SectionLabel icon={Settings2} label={tr("Konto", "Account")} />
          ...
          {!(isFoundingUser && daysRemaining === -1) && (
            <div className="bg-card rounded-2xl shadow-sm overflow-hidden mt-2 px-4 py-3.5">
              <div className="flex items-center justify-between">
          ...
          {!showLogoutConfirm ? (
            <button onClick={() => setShowLogoutConfirm(true)}
              className="w-full flex items-center justify-center gap-3 p-3.5 mt-2 bg-destructive/10 text-destructive rounded-2xl shadow-sm hover:bg-destructive/20 transition-colors active:scale-[0.98] font-semibold text-sm"
            >
              <LogOut className="w-4 h-4" />
              {t("settings.logout")}
            </button>
          ) : (
            <div className="bg-card rounded-2xl shadow-sm p-4 mt-2 space-y-3 border border-destructive/20">
        ...
        <section className="animate-reveal-up" style={{ animationDelay: "120ms" }}>
          <SectionLabel icon={Lock} label={t("settings.legal")} />
          <div className="bg-card rounded-2xl shadow-sm overflow-hidden">
