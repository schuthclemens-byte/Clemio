...
BEGIN
  ...
  IF _severity IS NOT NULL AND _severity NOT IN ('warning', 'error', 'fatal', 'problematic') THEN
  ...
  WITH filtered AS (
    ...
    WHERE (_status IS NULL OR e.status = _status)
      AND (_severity IS NULL OR e.severity = _severity OR (_severity = 'problematic' AND e.severity IN ('error', 'fatal')))
