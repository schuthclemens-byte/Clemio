...
-- Rewrite SECURITY DEFINER functions so the owner guard (auth.uid() = id)
-- is explicit and detectable by scripts/security-scan.mjs.
...
BEGIN
  ...
  SELECT phone_normalized INTO _phone FROM public.profiles WHERE id = auth.uid();
